...
//				running = false;
	...
	public Point getPathBFS(int x, int y) {
		...
		ar = getCheckMaze();
//		for (int q = 0; q < checkMaze.length; q++) {
//			for (int w = 0; w < checkMaze[0].length; j++) {
//				
//				if (ar[q][w] == -1 || ar[q][w] == 9) {
//					ar[q][w] = 0;
//				}
...
//			}
	...
	public void greenMovement() {
		...
		if (greenEnemy.getTranslateX() % 40 == 0 && greenEnemy.getTranslateY() % 40 == 0) {
			//System.out.println(greeny + " " + greenx);
			//System.out.println(greenEnemy.getTranslateY() + " " + greenEnemy.getTranslateX());
...
//		}
	...
	public void purpleMovement() {
		...
		System.out.println(purpleEnemy.getTranslateY() + " " + purpleEnemy.getTranslateX());
	...
	private void moveuserX(int value) {
		...
		for (int i = 0; i < Math.abs(value); i++) {
			for (Node platform : platforms) {
				// System.out.println(user.getScaleX());
